...
| Intent Router | ConditionAgent | Routes to appropriate agent based on intent |
| Agent.DataFetcher | Agent | SmartSheets API integration & data retrieval |
| Agent.ChangeDetector | Agent | Snapshot comparison & delta identification |
| Agent.HeatMapAnalyzer | Agent | Update frequency tracking & hot issue detection |
| Agent.StatusTransitionTracker | Agent | Workflow metrics & lifecycle tracking |
| Agent.ReportGenerator | Agent | Comprehensive Markdown report generation |
| Agent.QueryHandler | Agent | Ad-hoc queries & drill-down analysis |
| Agent.AlertManager | Agent | Threshold monitoring & critical issue flagging |
| Agent.TrendAnalyzer | Agent | Pattern recognition & predictive insights |
